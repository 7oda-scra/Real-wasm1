...
## Phase 8: Dynamic CRUD Architecture
- **Shared HR DTO Layer**: Introduced `BaseDto` in `Shared/Models/HrDtos.cs` as the canonical `Id` + `Name` contract for lightweight master data screens. `Sector`, `Department`, `Job`, `Qualification`, and `Section` now inherit from that base model to keep the CRUD surface strongly typed while preserving a single reusable form schema.
- **Attendance Domain Modeling**: Added `AttendanceSystem` with an explicit `AttendanceTrackingType` enum enforcing a mutually exclusive selection between `FingerprintRequired` and `OpenShift`. Added `Shift` with `AttendanceSystemId`, `StartTime`, `EndTime`, `WorkingHours`, and `ClosesNextDay` so attendance and scheduling can be composed without leaking UI-only state into the view layer.
- **Mock Data Service (`HrMockDataService`)**: Centralized HR seed data inside the Client project as a dedicated mock service. The service supplies curated lists for all HR setup forms, giving the WASM UI a single predictable source for demo data while keeping the DTO definitions in `Shared`.
- **Generic Wrapper (`DynamicCrudView.razor`)**: Replaced one-off HR form scaffolding with a reusable generic CRUD shell parameterized as `<DynamicCrudView TItem="class">`. The wrapper standardizes a premium two-part composition: a bordered, hoverable `MudTable` fed by `IEnumerable<TItem>` plus caller-provided `HeaderContent` and `RowTemplate`, followed by a rounded `MudPaper` form container embedding `FormActionToolbar` and a `ChildContent` slot for typed form inputs.
- **HR Screen Composition (`Hr.razor`)**: Each HR setup screen now projects its specific fields into the generic CRUD wrapper. Basic entities reuse the same ID/Name layout, attendance systems add an exclusive tracking-mode selector, and shifts combine attendance lookup, time pickers, numeric working hours, and next-day closing behavior inside a responsive `MudGrid`.
- **Design System Consistency**: Standardized HR input controls around `Variant="Variant.Outlined"` and `Margin="Margin.Dense"` while global CSS classes define the premium SaaS visual language: muted borders, soft gradients, rounded surfaces, elevated empty states, and polished table hover behavior.

## Phase 10: Mobile-First UX Overhaul
- **Mandate**: All future modules and components MUST adhere to the Drill-Down Master-Detail pattern on mobile, utilize horizontally scrolling tables, and implement responsive padding.
- **Responsive App Shell**: The global shell now relies on a breakpoint-aware `MudDrawer` configured with `Variant="DrawerVariant.Responsive"` and `Breakpoint="Breakpoint.Md"`, allowing desktop persistence while shifting into an overlay drawer on compact devices.
- **Navigation Behavior**: Mobile navigation now auto-dismisses the drawer after route changes, preserving focus and preventing the overlay from trapping the user after a selection.
